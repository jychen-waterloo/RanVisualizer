...
void AnimationState::Update(
    ...
    for (size_t i = 0; i < displayedBands_.size(); ++i) {
        ...
        const float target = std::clamp(mappedBands_[i] * profile.gain * lowLift, 0.0f, 1.0f);

        const float alpha = target > displayedBands_[i] ? riseAlpha : fallAlpha;
        displayedBands_[i] += (target - displayedBands_[i]) * alpha;

        const float peakTarget = std::clamp((i < mappedPeaks_.size() ? mappedPeaks_[i] : target) * lowLift, 0.0f, 1.0f);
